...
contract TestGasPriceFeesHook is Test, Deployers {
    ...
    // initially set when the hook was deployed.
    // 2. Conduct a swap at gas price exactly = 10 gwei, and this should charge us the BASE_FEE and
    // should keep the moving average at 10 gwei.
    // 3. Do a second swap - this time at 4 gwei - which should trigger higher fees in case of low
    // gas prices. Also, should update the moving average from 10 to 8 gwei.
    // 4. Do a third swap - this time at 12 gwei - which should trigger lower fees.
    // Also, it should update the moving average from 8 to 9 gwei now.
    // Keep swap amounts very small compared to the available liquidity so that the difference
    ...
    function test_feeUpdatesWithGasPrice() public {
        ...
        // Moving average shouldn't have changed
        ...
        // Moving average should now be (10 + 10 + 4) / 3 = 8 Gwei
        movingAverageGasPrice = hook.movingAverageGasPrice();
        movingAverageGasPriceCount = hook.movingAverageGasPriceCount();
        ...
        // Moving average should now be (10 + 10 + 4 + 12) / 4 = 9 Gwei
        movingAverageGasPrice = hook.movingAverageGasPrice();
        movingAverageGasPriceCount = hook.movingAverageGasPriceCount();
